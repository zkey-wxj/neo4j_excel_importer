...
export default function DetailPanel() {
  ...
  const handleEdit = () => {
    // OpsPanel watches selectedNodeId, so setting it will fill the form
    ...
    >
      <ScrollArea className="max-h-[calc(100dvh-80px)]">
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {(Array.isArray(raw.labels) ? raw.labels : [nodeType]).map((label, i, arr) => (
            <Badge
              ...
              variant="outline"
              className={cn(
                'text-[9.5px] tracking-widest font-semibold border-transparent',
                i === 0 && 'pl-0',
                i === arr.length - 1 && 'pr-0',
              )}
              style={{ color: nodeColor }}
        ...
        <div className="flex items-center gap-1.5 mb-4">
          ...
          <code className="text-[11px] font-semibold bg-muted/60 px-1.5 py-0.5 rounded break-all">
            {nodeNid}
          </code>
        </div>
        ...
        <div className="mb-3">
          <div className="text-[10px] text-muted-foreground tracking-wide mb-1.5 border-b border-border pb-1">
            属性 / 描述
          </div>
          {raw.description && (
            <div className="flex justify-between py-0.5 text-[11px]">
              ...
              <span className="font-semibold text-right max-w-[60%] break-all">
                {clean(raw.description).slice(0, 80)}
              </span>
            </div>
          )}
          {propEntries.map(([k, v]) => (
        ...
        <div className="mb-3">
          ...
          <div className="max-h-[200px] overflow-y-auto">
            {neighbors.length === 0 && (
              <div className="text-[10px] text-muted-foreground/60 py-1">无关联节点</div>
            )}
            {neighbors.slice(0, 30).map((nb, idx) => (
              <div
                key={idx}
                onClick={() => handleNavigate(nb.node)}
                className={cn(
                  'flex justify-between items-center',
                  'py-1 px-1 rounded-md cursor-pointer',
                  'text-[10.5px] hover:bg-muted/50 transition-colors'
                )}
              >
                <span className="truncate max-w-[70%]">
                  {nb.dir} {nb.node.label}
                </span>
                <span className="text-muted-foreground text-[9.5px] shrink-0 ml-2">
                  {nb.rel}
                </span>
              </div>
            ))}
